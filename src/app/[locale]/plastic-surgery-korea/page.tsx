...
const relatedHrefs = ["/hair-transplant-korea", "/aesthetic-clinic-seoul", "/how-to-choose-hospital-abroad", "/how-it-works"]

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params
  const dict = await getDictionary(locale as Locale)
  const t = dict.plasticSurgery.meta
  return {
    title: t.title, description: t.description,
    alternates: { canonical: `https://aetherheal.com/${locale}/plastic-surgery-korea`, languages: Object.fromEntries(locales.map((l) => [l, `https://aetherheal.com/${l}/plastic-surgery-korea`])) },
    openGraph: { title: t.title, description: t.description, url: `https://aetherheal.com/${locale}/plastic-surgery-korea`, images: [{ url: "/og-image.png", width: 1200, height: 630 }] },
    twitter: { card: "summary_large_image", title: t.title, description: t.description },
  }
}

export default async function PlasticSurgeryKoreaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const dict = await getDictionary(locale as Locale)
  const t = dict.plasticSurgery
  const prefix = `/${locale}`
            ...
            <div className="space-y-24">
              <div id="common-procedures" className="scroll-mt-32">
                ...
                <div className="space-y-6 text-lg text-slate-600 font-light leading-[1.8]">
                  ...
                  <p className="font-medium text-brand-navy border-l-2 border-brand-gold pl-5 py-2">{t.commonProcedures.callout}</p>
                </div>
              </div>
              <div id="risk-of-unstructured" className="scroll-mt-32">
                <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-8 pb-4 border-b border-slate-100 leading-tight">{t.riskOfUnstructured.title}</h2>
                <div className="space-y-6 text-lg text-slate-600 font-light leading-[1.8]">
                  <p>{t.riskOfUnstructured.text}</p>
                  <div className="p-6 sm:p-8 bg-brand-navy/[0.02] border border-slate-100 rounded-2xl"><p className="text-slate-700 m-0">{t.riskOfUnstructured.callout}</p></div>
                </div>
              </div>
              <div id="how-aetherheal-helps" className="scroll-mt-32">
                <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-8 pb-4 border-b border-slate-100 leading-tight">{t.howWeHelp.title}</h2>
                <div className="space-y-6 text-lg text-slate-600 font-light leading-[1.8]">
                  <div className="grid gap-4">{t.howWeHelp.items.map((item) => (<div key={item} className="flex items-start gap-4 p-5 bg-white border border-slate-100 rounded-xl shadow-sm"><CheckCircle2 className="w-5 h-5 text-brand-gold shrink-0 mt-0.5" /><span className="text-brand-navy font-medium text-base">{item}</span></div>))}</div>
                </div>
              </div>
            </div>
          </div>
