    ...
    <div className="min-h-full">
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-white border-b border-slate-100">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-8">{t.market.title}</h2>
          <div className="space-y-6 text-lg text-slate-600 font-light leading-[1.8] mb-12">
            <p>{t.market.p1}</p>
            <p>{t.market.p2}</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {t.market.stats.map((stat: { number: string; label: string }) => (
              <div key={stat.label} className="rounded-2xl border border-brand-gold/20 bg-brand-gold/5 p-6 text-center">
                <p className="font-serif text-3xl sm:text-4xl text-brand-navy font-bold mb-2">{stat.number}</p>
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-bg-light border-b border-slate-100">
        <div className="max-w-4xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-8">{t.problem.title}</h2>
          <div className="space-y-6 text-lg text-slate-600 font-light leading-[1.8] mb-12">
            <p>{t.problem.p1}</p>
            <p>{t.problem.p2}</p>
          </div>
          ...
          <div className="rounded-2xl border border-red-200/60 bg-white p-6 sm:p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-serif text-lg text-brand-navy font-semibold">레몬 시장 사이클</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {t.problem.cycle.map((step: string, i: number) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-red-50/50">
                  ...
                  <span className="text-sm text-text-body leading-relaxed">{step}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-white border-b border-slate-100">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-4">{t.model.title}</h2>
          <p className="text-lg text-slate-600 font-light leading-[1.8] mb-12">{t.model.p1}</p>
            ...
            {t.model.revenue.map((item: { title: string; description: string }, i: number) => {
              ...
              return (
                <div key={item.title} className="rounded-2xl border border-slate-200 bg-bg-light p-7">
                  ...
                  <h3 className="font-serif text-lg text-brand-navy font-semibold mb-3">{item.title}</h3>
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-bg-light border-b border-slate-100">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-12 text-center">{t.moat.title}</h2>
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-white border-b border-slate-100">
        <div className="max-w-4xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-12 text-center">{t.team.title}</h2>
          <div className="rounded-3xl border border-slate-200 bg-bg-light p-8 sm:p-10 max-w-2xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="font-serif text-xl text-brand-navy font-semibold">{t.team.founder.name}</h3>
                <p className="text-sm text-brand-gold font-medium">{t.team.founder.title}</p>
              </div>
            </div>
            <p className="text-sm text-text-body leading-relaxed">{t.team.founder.bio}</p>
          </div>
        </div>
      </section>
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-bg-light border-b border-slate-100">
        <div className="max-w-4xl mx-auto">
          <h2 className="font-serif text-3xl sm:text-4xl text-brand-navy mb-12 text-center">{t.traction.title}</h2>
          <div className="space-y-4 max-w-2xl mx-auto">
            {t.traction.milestones.map((m: { status: string; text: string }, i: number) => (
              <div key={i} className="flex items-start gap-4">
                ...
                <span className={`text-sm leading-relaxed ${m.status === "done" ? "text-text-body" : m.status === "current" ? "text-brand-navy font-semibold" : "text-text-muted"}`}>
                  {m.text}
                </span>
              </div>
            ))}
      ...
      <section className="w-full py-20 sm:py-28 px-4 sm:px-6 bg-brand-navy">
        <div className="max-w-3xl mx-auto text-center space-y-8">
          ...
          <div className="pt-8">
            <Link href={`${prefix}/our-philosophy`} className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-brand-gold transition-colors">
              우리의 철학 읽기
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
